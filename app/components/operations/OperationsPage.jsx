...
import TeamReviewMarquee from "./TeamReviewMarquee";
...
function AISection() {
  ...
  return <section className="pm-section pm-ai"><div className="pm-shell">
    ...
    <div className="pm-ai-grid">{cards.map(([title, body, bold, image]) => <article key={title}><Image src={`/operations/${image}`} alt={title} width={560} height={420} /><h3>{title}</h3><p>{body}{bold && <strong>{bold}</strong>}</p></article>)}</div>
  </div></section>;
}
...
function FeatureCard([title, body, image]) {
  return <article key={title}>
    ...
    <div className="pm-card-image"><Image src={`/operations/${image}`} alt={title} width={900} height={680} /></div>
  </article>;
}
  ...
  return <>
    ...
    <section className="pm-integrations">
      ...
      <div className="pm-marquee">
        ...
        <span className="pm-marquee-badge"><Image src="/operations/comparison-logo-mark.svg" alt="Hashboard" width={130} height={155} className="pm-marquee-mark" /></span>
      </div>
    </section>
    <section className="pm-templates"><div className="pm-shell">
      ...
      <div className="pm-tpl-grid">{[["Project Management Template", "gantt.png"], ["Agile Management Template", "dashboards.png"], ["Team Requests Template", "forms.png"], ["Scope of Work Template", "docs.png"], ["OKRs Template", "workload.png"], ["Project Charter Template", "templates.png"]].map(([name, img]) => <Link href="/signup" key={name}>
        <span className="pm-tpl-shot"><Image src={`/operations/${img}`} alt={name} width={900} height={620} /></span>
        <h3>{name}</h3>
      </Link>)}</div>
    </div></section>
  </>;
...
function Resources() {
  ...
  return <section className="pm-resources"><div className="pm-shell">
    ...
    <div className="pm-res-grid">{posts.map(([title, author, role, art, tone]) => <article key={title}>
      <span className="pm-res-art"><Image src={`/operations/${art}`} alt="" width={380} height={240} /></span>
      <h3>{title}</h3>
      <div className="pm-res-by">
        {tone === "brand"
          ? <span className="pm-res-avatar pm-res-avatar-brand"><Image src="/operations/comparison-logo-mark.svg" alt="" width={130} height={155} /></span>
...
export default function OperationsPage() {
  return <div className="pm-page"><Hero /><OldNew /><AISection />{groups.map((group, i) => <FeatureGroup group={group} index={i} key={group.title} />)}<TestimonialAndMore /><Resources /><TeamCtaBanner /><TeamReviewMarquee /><FAQ /></div>;
}
