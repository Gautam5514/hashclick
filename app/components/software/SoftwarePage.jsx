...
const groups = [
  ...
  },
  {
    ...
    cards: [
      ["Sync tasks with Git tools", "Connect commits, branches, and pull requests to ClickUp tasks from GitHub, GitLab, and Bitbucket.", "git.png"],
      ["Migrate Jira seamlessly", "Easily import and manage Jira issues directly in ClickUp.", "jira.png"],
...
const faqs = [
  ["How does ClickUp compare with other project management tools for software teams?", "ClickUp centralizes tasks, sprints, and bug tracking while integrating the DevOps pipeline into one workspace, streamlining work from planning through deployment."],
  ["Can ClickUp help software teams track their DevOps performance effectively?", "Yes. Customizable Dashboards and direct DevOps integrations help teams monitor lead time, deployment frequency, recovery time, and bottlenecks."],
];
...
function Hero() {
  return <section className="pm-hero sw-hero"><div className="pm-shell pm-hero-grid"><div className="pm-hero-copy"><p className="pm-kicker"><Code2 /> AI-powered software management</p><h1>The everything app for <span>software teams</span></h1><p className="pm-lede">Plan, build, and ship all in one place. ClickUp centralizes your entire engineering lifecycle, from roadmap to release, so your teams ship faster without roadblocks.</p><div className="pm-action-row"><CTA /><span>Free forever.<br />No credit card.</span></div><div className="pm-reviews"><b>★★★★★</b><span>4.6 stars · 25,000+ reviews</span></div></div><div className="pm-hero-art"><Image src="/software/hero.png" alt="ClickUp software team List view" width={1460} height={828} priority /></div></div></section>;
}
...
function Comparison() {
  ...
  return <section className="pm-dark"><div className="pm-shell"><header className="pm-center"><p className="pm-overline">The ClickUp way</p><h2>Product management is chaotic,<br /><em>let&apos;s fix it</em></h2><p>Disconnected tools sabotage software teams. ClickUp unifies your backlog, bug tracking, sprints, and roadmaps—so your teams can collaborate, build, and ship faster.</p></header><div className="pm-compare">{[["The old way", old, false], ["The ClickUp way", better, true]].map(([title, list, good]) => <article className={good ? "good" : "bad"} key={title}><h3>{title}</h3><ul>{list.map(item => <li key={item}>{good ? <Check /> : <X />}{item}</li>)}</ul></article>)}</div><div className="pm-center"><CTA light /></div></div></section>;
}
